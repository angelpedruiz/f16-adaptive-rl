...
class PendulumCartTrainer:
    ...
    def train_hdp(self, agent: HDPAgent, max_steps: int) -> dict:
        ...
        training_data = {
            ...
            "model_predictions": [],
            "true_states": [],
            "losses": {"actor": [], "critic": [], "model": []},
            "weight_norms": {"actor": [], "critic": [], "model": []},
            "weight_update_norms": {"actor": [], "critic": [], "model": []},
            "gradient_norms": {"actor": [], "critic": [], "model": []},
        }
        ...
        for step in tqdm.tqdm(range(max_steps), desc="Training HDP Agent", unit="step"):
            ...
            action = agent.get_action(obs) # no noise, [-1,1]
            action = action + np.random.normal(0, 0.01, size=action.shape)  # add exploration noise
            ...
            if step % 10 == 0:
                ...
                for layer in agent.actor.model[:-1]:  # all layers except last tanh
                    ...
                    if isinstance(layer, torch.nn.Linear):
                        print(
                            "Actor layer pre-activation mean/std:",
                            x.mean().item(),
                            x.std().item(),
                        )
    ...
    def train_ihdp(self, agent, max_steps: int) -> dict:
        ...
        for step in tqdm.tqdm(
            range(max_steps), desc="Training IHDP Agent", unit="step"
        ):
...
if __name__ == "__main__":
    ...
    # # Render animation of the episode (adjustable playback speed and frame skipping)
    # plotting_manager.render_pendulumcart_env(
    #     states=training_data["states"],
    #     filename="pendulum_animation.gif",
    #     speed=0.5,  # 1.0 = real-time, 0.5 = slower, 2.0 = faster
    #     skip_frames=5,  # 1 = every frame, 5 = every 5th frame (smaller file, faster plotting)
    # )
