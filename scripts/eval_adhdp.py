...
if __name__ == "__main__":
    ...
    # For ADHDP, we typically store:
    # - returns (rewards per episode)
    # - lengths (episode lengths)
    # - training_error (actor/critic TD error)
    ...
    try:
        ...
        print("❌ Error: config/adhdp.yaml not found.")
        sys.exit(1)
    ...
    terminations = np.ones_like(rewards, dtype=bool)  # Assume all episodes terminated normally
    ...
    # ADHDP-specific attributes: reward queue, length queue, training_error
    class DummyEnv:
        return_queue = returns
        length_queue = lengths

    class DummyAgent:
        training_error = training_error  # actor/critic TD error

    # === Plot the results ===
    rolling_length = eval_window
    plot_path = exp_dir / f"training_summary_adhdp_{rolling_length}.png"
