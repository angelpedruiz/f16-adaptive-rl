...
class IQNCritic(nn.Module):
    """
    IQN Citic for SAC (distributional).
    Predicts multiple quantiles for each state-action pair
    """
    def __init__(self, obs_dim: int, act_dim: int, hidden_sizes: list[int], n_quantiles: int = 32, embedding_dim: int = 64):
        super().__init__()
        self.n_quantiles = n_quantiles
        self.embedding_dim = embedding_dim
        
        # Quantile embedding MLP
        self.quantile_fc= nn.Linear(embedding_dim, hidden_sizes[0])
        
        # Build Q1 network
        layers = []
        input_dim = hidden_sizes[0] # after combining obs + quantile
        for h in hidden_sizes[1:]:
            layers.append(nn.Linear(input_dim, h))
            layers.append(nn.ReLU())
            input_dim = h
        layers.append(nn.Linear(input_dim, 1)) # output one quantile
    ...
    def forward(self, obs, act):
        ...
        Returns: quantiles [batch_size, n_quantiles, 1] for Q1 and Q2
        '''
        batch_size = obs.shape[0]
        
        # Sample tau ~ U(0, 1)
        tau = torch.rand(batch_size, self.n_quantiles, 1, device=obs.device)  # [batch_size, n_quantiles, 1]
        
        # Cosine ebedding of tau
        pi = torch.acos(torch.tensor(-1.0))
        i = torch.arange(1, self.embedding_dim + 1, device=obs.device).float()  # [embedding_dim]
        tau_embed = torch.cos(pi * i * tau)  # [batch_size, n_quantiles, embedding_dim]
        tau_embed = F.relu(self.quantile_fc(tau_embed)) # [batch_size, n_quantiles, hidden_sizes[0]]
        
        # Expand obs and act to match n_quantiles
        obs_exp = obs.unsqueeze(1).expand(-1, self.n_quantiles, -1)  # [batch_size, n_quantiles, obs_dim]
        act_exp = act.unsqueeze(1).expand(-1, self.n_quantiles, -1)  # [batch_size, n_quantiles, act_dim]

        x = torch.cat([obs_exp, act_exp, tau_embed], dim=-1)  # [batch_size, n_quantiles, obs_dim + act_dim + hidden_sizes[0]]
        
        x_flat = x.view(batch_size * self.n_quantiles, -1)  # [batch_size * n_quantiles, ...]
        q1 = self.Q1(x_flat).view(batch_size, self.n_quantiles, 1)  # [batch_size, n_quantiles, 1]
        q2 = self.Q2(x_flat).view(batch_size, self.n_quantiles, 1)  # [batch_size, n_quantiles, 1]
...
class SACAgent:
    ...
    env: Optional[object] = None):
        ...
        self.critic = IQNCritic(self.obs_dim, self.act_dim, hidden_sizes).to(self.device)
        self.critic_target = IQNCritic(self.obs_dim, self.act_dim, hidden_sizes).to(self.device)
    ...
    def update(self, obs, action, reward, terminated, next_obs):
        ...
        # ------------------------ Critic Update ------------------------ #
        with torch.no_grad(): # 2. CALC LOSS
            # Next action and log-prob form target actor
            next_action, next_log_prob = self.actor.sample(next_obs_batch)
            # Target quantiles
            target_q1, target_q2 = self.critic_target(next_obs_batch, next_action) # [batch_size, n_quantiles, 1]
            target_v = torch.min(target_q1, target_q2) - self.alpha.detach() * next_log_prob.unsqueeze(-1)  # [batch_size, n_quantiles, 1]
            target_q = reward_batch + self.gamma * target_v * not_dones_batch

        # Compute current Q estimates
        current_q1, current_q2 = self.critic(obs_batch, action_batch) # 1.PREDICT
        critic_loss = F.mse_loss(current_q1, target_q) + F.mse_loss(current_q2, target_q) # 2.CALC LOSS
    ...
    def _quantile_huber_loss(self, pred, target, taus, kappa=1.0):
        ...
        pred:   [batch_size, N, 1]  predicted quantiles
        target: [batch_size, N', 1] target quantiles
        taus:   [batch_size, N, 1]  quantile fractions used for pred
        kappa:  float, Huber threshold
        """
        # Expand to pairwise differences between each predicted and target quantile
        diff = target.unsqueeze(1) - pred.unsqueeze(2)   # [batch, N, N', 1]
        abs_diff = diff.abs()
        
        # Compute Huber component
        huber = torch.where(abs_diff <= kappa,
                            0.5 * diff.pow(2),
                            kappa * (abs_diff - 0.5 * kappa))
        
        # Compute quantile weights
        weight = (taus.unsqueeze(2) - (diff.detach() < 0).float()).abs()  # [batch, N, N', 1]
        
        # Weighted loss
        loss = (weight * huber).mean()  # average over all dimensions
